...
public class BuscarAlarma extends AppCompatActivity implements DatePickerDialog.OnDateSetListener {
    ...
    public void cargarDatos() {
        ...
        Cursor alarmaBuscada = obtenerAlarma();

        //La BD empieza desde 1
        if (alarmaBuscada.moveToFirst()) {
            //Con esto conseguimos que los timePicker inicien con la hora puesta por el usuario
            periodoHoras = alarmaBuscada.getInt(3);
            periodoMinutos = alarmaBuscada.getInt(4);

            nombreMedicina.setText(alarmaBuscada.getString(2));
            periodoMedicina.setText(String.format("%02d:%02d", periodoHoras, periodoMinutos));
            notasMedicina.setText(alarmaBuscada.getString(5));
            mostrarFecha.setText(alarmaBuscada.getString(6));
            horaMedicina.setText(alarmaBuscada.getString(7));
            mostrarFechaFinal.setText(alarmaBuscada.getString(8));

            //traemos el valor de activar para saber mostrar si esta activada o no la alarma
            int activar = alarmaBuscada.getInt(1);
            if (activar == 1) {
    ...
    public void modificar(View view) {
        ...
        int opcion = alarmaBuscada.getInt(0);

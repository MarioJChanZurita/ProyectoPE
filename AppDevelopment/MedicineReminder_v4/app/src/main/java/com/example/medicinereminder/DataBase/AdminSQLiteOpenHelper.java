...
public class AdminSQLiteOpenHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase BaseDatos) {
        BaseDatos.execSQL("create table datos(" +
                "posicion int primary key, activar int, nombre text, periodoHoras int, periodoMinutos int, notas text, hora time, fechaFinal date)");
        /**
    ...
    public Cursor obtenerAlarmaActivarse (String hora_sistema){
        SQLiteDatabase baseDatos = getWritableDatabase(); //Abre la base de datos en modo escritura
        Cursor alarmaActivarse = baseDatos.rawQuery("SELECT * FROM datos WHERE activar='"+1+"' AND hora = '"+hora_sistema+"'",null);
